...
### 삭제 규칙 정의
고아 레코드를 방지하는데 도움이 됨
- 거부 : 부모테이블 레코드를 삭제하지 않고 유지하고 비활성으로 지정
- 제약 : 자식테이블에 연관된 레코드가 있을 시, 부모 레코드를 삭제 하지 않음
- 연속 : 부모레코드를 삭제하고 자식테이블의 모든 연관된 레코드를 지움
- 널화  :  부모레코드를 삭제하고 자식테이블의 연관된 레코드의 외래키 값을 널로 갱신함
- 기본값 설정 : 부모레코드를 삭제하고  자식테이블의 연관된 레코드의 외래키 값을 필드명세의 현재 기본값으로 갱신함